...
/// Rejects metadata application where lossless preservation is unsupported.
#[cfg(not(unix))]
pub fn apply_metadata(dest: &Path, _source_meta: &Metadata, _is_symlink: bool) -> Result<()> {
    anyhow::bail!("Lossless metadata preservation is not implemented on this platform: {}", dest.display())
}
