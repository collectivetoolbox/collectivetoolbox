...
pub async fn get_nodes_index(
    ...
    let nodes_list: Vec<_> = nodes
        .into_iter()
        .map(|node| {
            let preview = ctb_formats_dcstring::dctext::format_blob_preview(
                &node.data,
...
pub async fn get_nodes_view(
    ...
    let display_content = if is_statements {
        let dctext_bytes =
            ctb_formats_dcstring::dctext::dcutf_to_dctext(node.data.clone());
...
mod tests {
    ...
    async fn test_publish_and_redirect_flow() {
        ...
        let dctext_bytes =
            ctb_formats_dcstring::dctext::dcutf_to_dctext(updated_node.data);
