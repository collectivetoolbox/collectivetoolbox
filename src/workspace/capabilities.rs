//! Capability set definitions for the workspace.
//!
...
pub fn create_io_capabilities() -> CapabilitySet {
    let mut caps =
        ctb_workspace_ipc::auth::capability::default_service_capabilities(
            ChildKind::Io,
        );

    // IO needs to request spawns and make proxy calls to storage child services
    caps.allowed.insert(
        ServiceName(PARENT_SERVICE_NAME.to_string()),
        vec![
            MethodRule {
    ...
    caps.allowed
        .entry(ServiceName("workspace".to_string()))
        .or_default()
        .extend([
            MethodRule {
                method: MethodSelector::Exact("get_crlite_artifact".into()),
                quotas: None,
            },
            MethodRule {
                method: MethodSelector::Exact("get_crlite_cache_status".into()),
                quotas: None,
            },
            MethodRule {
                method: MethodSelector::Exact(
                    "get_crlite_manifest_json".into(),
                ),
                quotas: None,
            },
            MethodRule {
                method: MethodSelector::Exact("get_update_status".into()),
                quotas: None,
            },
        ]);

    caps
    // TODO: read stdin; keyboard, mouse, etc. if configured; input/output other local input (USB, serial, etc) if configured; listen on 0.0.0.0 if the webUI is configured to do so
}

/// Create capabilities for the renderer process.
pub fn create_storage_capabilities() -> CapabilitySet {
    ctb_workspace_ipc::auth::capability::default_service_capabilities(
        ChildKind::Storage,
    )
    // TODO: read/write files, sandboxed to the workspace's data directory
}
