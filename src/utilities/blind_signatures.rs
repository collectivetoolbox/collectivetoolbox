...
pub use voprf::{Ristretto255, OprfServer, OprfClient, BlindedElement, EvaluationElement};
...
pub fn generate_server_key() -> Vec<u8> {
    ...
    let server = OprfServer::<Ristretto255>::new(&mut rng).expect("Failed to generate server key");
...
pub fn deserialize_blinded(bytes: &[u8]) -> Result<BlindedElement<Ristretto255>> {
...
pub fn serialize_evaluation(evaluation: &EvaluationElement<Ristretto255>) -> Vec<u8> {
    evaluation.serialize().to_vec()
}

/// Helper to deserialize an EvaluationElement from bytes.
pub fn deserialize_evaluation(bytes: &[u8]) -> Result<EvaluationElement<Ristretto255>> {
...
pub fn server_evaluate(server_key: &[u8], blinded_element_bytes: &[u8]) -> Result<Vec<u8>> {
...
pub fn client_finalize(serial: &[u8], client_state_bytes: &[u8], evaluation_element_bytes: &[u8]) -> Result<Vec<u8>> {
    ...
    let output = client.finalize(serial, &evaluation)
...
pub fn server_verify(server_key: &[u8], serial: &[u8], token_tag: &[u8]) -> Result<bool> {
    let server = OprfServer::<Ristretto255>::deserialize(server_key)
        .map_err(|e| anyhow!("Failed to deserialize server key: {:?}", e))?;

    let expected = server.evaluate(serial)
...
mod tests {
    ...
    fn test_voprf_flow() -> Result<()> {
        ...
        let is_valid_diff = server_verify(&server_key, b"different-serial", &token_tag)?;
