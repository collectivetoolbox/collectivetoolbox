...
use crate::https::{async_client, ClientOptions};
...
pub async fn replenish_tokens(server_url: &str, user_session_token: &str, count: usize) -> Result<()> {
    if count == 0 {
        return Ok(());
    }

    let user_id = ipcb!(storage).validate_session_b(user_session_token)?
    ...
    for _ in 0..count {
        ...
        blinded_elements.push(base64::engine::general_purpose::STANDARD.encode(blinded));
    ...
    // Finalize tokens
    for i in 0..count {
        let serial = serials.get(i).ok_or_else(|| anyhow!("Serial out of bounds"))?;
        let state = client_states.get(i).ok_or_else(|| anyhow!("State out of bounds"))?;
        let eval_str = body.evaluations.get(i).ok_or_else(|| anyhow!("Evaluation out of bounds"))?;
        let eval_bytes = base64::engine::general_purpose::STANDARD.decode(eval_str)?;
        ...
        ipcb!(storage).save_local_token_b(user_id, key_val, &format!("{serial_hex}:{tag_hex}"))?;
    }
...
pub async fn start_sync_session(server_url: &str, user_id: u64) -> Result<String> {
    ...
    let serial_hex = parts.get(0).ok_or_else(|| anyhow!("Missing serial"))?.to_string();
    let token_tag_hex = parts.get(1).ok_or_else(|| anyhow!("Missing tag"))?.to_string();
    ...
    let resp = client.inner.post(&url)
        .json(&SyncStartRequest { serial_hex, token_tag_hex })
        .send()
        .await?;

    if !resp.status().is_success() {
        bail!("Failed to start sync session on server: status {}", resp.status());
    }
...
pub async fn reserve_ids_remote(user_id: u64, server_url: &str, session_id: &str, graph_id: u128) -> Result<IdRange> {
    let client = async_client(ClientOptions::default())?;
...
pub async fn allocate_local_id(
    ...
    let mut range: Option<IdRange> = ipcb!(storage).get_local_id_range_b(user_id, graph_id)?
        .and_then(|bytes| serde_json::from_slice(&bytes).ok());

    let current_range = match range.as_mut() {
        Some(r) => {
            if r.next_id > r.end_id {
                // Out of IDs! Must fetch synchronously
                let Some(sid) = session_id else {
                    return Err(anyhow!("No local IDs remaining and no active session to reserve more."));
                };
                let new_range = reserve_ids_remote(user_id, server_url, sid, graph_id).await?;
        ...
        None => {
            ...
            let Some(sid) = session_id else {
                return Err(anyhow!("No local ID range initialized for graph {graph_id} and no active session."));
            };
            let new_range = reserve_ids_remote(user_id, server_url, sid, graph_id).await?;
            range = Some(new_range);
            range.as_mut().ok_or_else(|| anyhow!("Failed to initialize range"))?
        }
    };
    ...
    let remaining = if current_range.end_id >= current_range.next_id {
        current_range.end_id.saturating_sub(current_range.next_id).saturating_add(1)
    } else {
        0
    };
    if remaining < 3000 {
        if let Some(sid) = session_id {
            ...
            tokio::spawn(async move {
                let _ = reserve_ids_remote(user_id, &server_url, &sid, graph_id).await;
            });
        }
    }
...
static MOCK_REGISTER_RESULTS: Mutex<Option<HashMap<String, RemoteRegisterResult>>> = Mutex::new(None);
...
pub async fn register_on_server(user_id: u64, server_url: &str) -> Result<RemoteRegisterResult> {
    ...
    let resp_res = client.inner.post(&url)
        .json(&req_body)
        .send()
        .await;
    ...
    if status_code == 201 || resp.status().is_success() {
        ...
        log!(format!("register_on_server: server returned error status: {status_code}"));
