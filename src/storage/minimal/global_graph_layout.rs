...
fn get_layout_table() -> Result<std::sync::Arc<csv_tools::CsvTable>> {
    csv_tools::get_or_load_cached(
        "ctb_storage_minimal::data/global-graph-layout.csv",
        || {
            let bytes = crate::get_storage_minimal_inline_data(
                "global-graph-layout.csv",
            )
            .ok_or_else(|| {
                anyhow::anyhow!("Missing embedded global-graph-layout.csv")
            })?;
