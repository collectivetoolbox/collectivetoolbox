...
pub static SYNC_MIGRATIONS: &[Migration] = &[
    Migration {
...
pub(crate) async fn check_column_exists(conn: &Connection, table: &str, column: &str) -> Result<bool> {
...
async fn run_rust_migration(conn: &Connection, schema_type: DbSchemaType, name: &str) -> Result<()> {
    match (schema_type, name) {
        ...
        (DbSchemaType::Users, m2026_07_02_1_add_user_subscription::NAME) => {
            m2026_07_02_1_add_user_subscription::run_rust_migration(conn).await?;
        }
        (DbSchemaType::Users, m2026_07_05_3_add_user_remote_status::NAME) => {
            m2026_07_05_3_add_user_remote_status::run_rust_migration(conn).await?;
        }
        ...
        (DbSchemaType::Nodes, m2026_07_05_1_add_client_sync_tables::NAME) => {
            m2026_07_05_1_add_client_sync_tables::run_rust_migration(conn).await?;
        }
        (DbSchemaType::Sync, m2026_07_05_2_create_server_sync_tables::NAME) => {
            m2026_07_05_2_create_server_sync_tables::run_rust_migration(conn).await?;
        }
        _ => {}
    }
    Ok(())
...
pub async fn run_migrations(conn: &Connection, schema_type: DbSchemaType) -> Result<()> {
    // Drop old format table if it exists with version column (from older development versions)
    let has_version_col = {
        let mut stmt = conn.prepare("PRAGMA table_info(schema_migrations)").await?;
        let mut rows = stmt.query(()).await?;
        let mut found = false;
        while let Some(row) = rows.next().await? {
            if let Ok(Value::Text(col_name)) = row.get_value(1) {
    ...
    let mut stmt = conn.prepare("SELECT name FROM schema_migrations ORDER BY name ASC").await?;
    let mut rows = stmt.query(()).await?;
