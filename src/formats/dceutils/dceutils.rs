...
pub fn dce_convert(
    ...
    let dc = match input_format {
        ...
        "3_0a" => dce3_0a::convert_3_0a_to_dc(data)?,
        "3_0a_raw" => dce3_0a::convert_3_0a_raw_to_dc(data)?,
        "hex_3_0a" => {
            let bin = (|| -> Result<Vec<u8>> {
                let hex_str = std::str::from_utf8(data)?;
                ctb_formats_hexdump::hex2bin(hex_str)
        ...
        "3_01a" => dce3_01a::convert_3_01a_to_dc(data)?,
        "3_01a_raw" => dce3_01a::convert_3_01a_raw_to_dc(data)?,
        "hex_3_01a" => {
            let bin = (|| -> Result<Vec<u8>> {
                let hex_str = std::str::from_utf8(data)?;
                ctb_formats_hexdump::hex2bin(hex_str)
        ...
        "legacy_cdce" => cdce::convert_legacy_cdce_to_dc(data, false)?,
        "cdce_lstrict" => cdce::convert_legacy_cdce_to_dc(data, true)?,
        "utf8" => unicode::convert_utf8_to_dc(data)?,
        "utf32" => unicode::convert_utf32_to_dc(data)?,
        "utf8_base64" => unicode::convert_utf8_base64_to_dc(data)?,
        "utf8_dc64" => unicode::convert_utf8_dc64_to_dc(data)?,
        "utf8_dc64_enc" => unicode::convert_utf8_dc64_enc_to_dc(data)?,
    ...
    match output_format {
        ...
        "dce" => dce3_0a::convert_dc_to_3_0a_output(&dc_cleaned),
        ...
        "hex_dce" => {
            let dce_bytes = dce_convert(dc_cleaned.as_bytes(), "dc", "dce")?;
            Ok(bin2hex(dce_bytes).to_uppercase().into_bytes())
        }
        "hex_3_0a" => {
            let bytes = dce_convert(dc_cleaned.as_bytes(), "dc", "3_0a")?;
            Ok(bin2hex(bytes).to_uppercase().into_bytes())
        }
        "hex_3_01a" => {
            let bytes = dce_convert(dc_cleaned.as_bytes(), "dc", "3_01a")?;
            Ok(bin2hex(bytes).to_uppercase().into_bytes())
        }
        "utf8" => unicode::convert_dc_to_utf8_output(&dc_cleaned),
