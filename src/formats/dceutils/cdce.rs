...
pub fn convert_legacy_cdce_to_dc(data: &[u8], strict: bool) -> Result<String> {
    ...
    while i < codepoints.len() {
        let cp = *codepoints.get(i).context("codepoints index out of bounds")?;
        if cp == 0x40 { // '@'
            // 1-character CDCE condition
            let mut matched_1char = false;
            let i_plus_2 = i.saturating_add(2);
            if i_plus_2 < codepoints.len() && codepoints.get(i_plus_2) == Some(&0x40) {
                let next_cp = *codepoints.get(i.saturating_add(1)).context("codepoints index out of bounds")?;
                if (0x31..=0x39).contains(&next_cp) { // '1'..='9'
            ...
            if !matched_1char {
                ...
                if i_plus_3 < codepoints.len() && codepoints.get(i_plus_3) == Some(&0x40) {
                    let cp1 = *codepoints.get(i.saturating_add(1)).context("codepoints index out of bounds")?;
                    let cp2 = *codepoints.get(i.saturating_add(2)).context("codepoints index out of bounds")?;
                    if (0x30..=0x39).contains(&cp1) && (0x30..=0x39).contains(&cp2) {
                ...
                if !matched_2char {
                    if strict {
                        ...
                        return Ok(format!("{}… CDCE decoding error!", cleaned));
                    } else {
...
pub fn convert_dc_to_legacy_cdce_output(data: &str) -> Result<Vec<u8>> {
    ...
    for dc_id in dc_array {
        if tables.cdce_html_legacy.contains_key(&dc_id) {
            ...
            let utf8_str = String::from_utf8(utf8_res).context("invalid UTF-8 from conversion")?;
