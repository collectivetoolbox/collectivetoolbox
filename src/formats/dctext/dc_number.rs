...
pub fn read_dc_number_global(gids: &[u128]) -> Result<(Integer, usize)> {
    ...
    while idx < gids.len() {
        ...
        } else if (SHORT_DC_BASE64_START..=SHORT_DC_BASE64_END).contains(&u32::try_from(gid).unwrap_or(0))
            || gid == u128::from(SHORT_DC_BASE64_PADDING)
        {
            let short_dc = u32::try_from(gid)?;
            short_dc_to_base64_char(short_dc)?
        } else {
