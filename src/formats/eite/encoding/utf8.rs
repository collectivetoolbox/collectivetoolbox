...
pub fn last_utf8_codepoint(bytes: &[u8]) -> (u32, usize) {
    ...
    for start in (0.max(len.saturating_sub(4))..len).rev() {
        // Reason for fallback: start is generated by range bounded by len, guaranteed start < len.
    ...
    // Fallback: last byte.
    // Reason for fallback: non-empty bytes check on line 75 guarantees len >= 1, so index len - 1 is within bounds.
    (
