...
| :--- | :--- |
| **`compress 1.0` / `2.0`** (1984) | Initial releases by Spencer W. Thomas et al. Basic unblocked LZW coding. `2.0` supported optional headerless streams (`nomagic`). |
| **`compress 3.0`** (Jan 1985) | Format revision by James A. Woods & Joe Orost. Standardized 3-byte header (`0x1F 0x9D`, `maxbits` + `BLOCK_MODE` bit) and byte-boundary code width alignment. Backwards incompatible with 2.0 output, but 3.0+ decoders decode 2.0 files. |
| **`compress 4.0`** (1985) | Standard System V / BSD world release. Optimized hash table performance and adaptive block-reset ratio checking. |
| **`compress 4.1`** | Added recursive directory compression (`-r` flag) by Dave Mack. |
| **`(N)compress 4.2+`** | Speed optimizations by Peter Jannesen featuring 2-level fast prime hash table lookup algorithm. |
| **`(N)compress 5.0` / `5.1`** | Modernization by Mike Frysinger. Standardized POSIX prototypes, added `-k` (keep input) flag, updated recursive directory handling, and dropped legacy `2.0` output generator flag (`-C`). |
| **`46.diff` (POSIX 2024 Updates)** | **Under consideration for POSIX Issue 8 (POSIX.1-2024) alignment**: Integrates POSIX `gzip`/`deflate` compression support via `zlib`. Adds `-g` and `-m algo` options, dual-purpose `-b` (maxbits vs gzip compression level 1–9), `.gz` file suffix support, magic byte auto-detection (`0x1F 0x8B`), build configuration flags (`USE_ZLIB` / `NO_ZLIB`), and exit code **`3`** for unsupported algorithm builds. |
