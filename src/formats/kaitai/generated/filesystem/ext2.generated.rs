...
pub struct Ext2 {
    pub(crate) _root: SharedType<Ext2>,
    pub(crate) _parent: SharedType<Ext2>,
...
pub struct Ext2_Bgd {
    pub(crate) _root: SharedType<Ext2>,
    pub(crate) _parent: SharedType<Ext2_BlockGroup>,
    pub(crate) _self_shared: SharedType<Self>,
...
impl Ext2_Bgd {
    ...
    pub fn inodes(
        ...
        *self.inodes.borrow_mut() = Vec::new();
        let l_inodes = usize::try_from(*self._root.get_value().borrow().upgrade().as_ref().ok_or(KError::MissingRoot)?.bg1()?.super_block().inodes_per_group())?;
        for _i in 0_usize..l_inodes {
            let t = Self::read_into::<_, Ext2_Inode>(&*_io, Some(self._root.clone()), Some(self._self_shared.clone()))?.into();
...
pub struct Ext2_BlockPtr {
    pub(crate) _root: SharedType<Ext2>,
    pub(crate) _parent: SharedType<Ext2_Inode>,
    pub(crate) _self_shared: SharedType<Self>,
    ptr: RefCell<u32>,
    _io: RefCell<BytesReader>,
...
impl Ext2_BlockPtr {
    pub fn body(
        ...
        *self.body_raw.borrow_mut() = _io.read_bytes(usize::try_from(*self._root.get_value().borrow().upgrade().as_ref().ok_or(KError::MissingRoot)?.bg1()?.super_block().block_size()?)?)?.into();
        ...
        let t = Self::read_into::<BytesReader, Ext2_RawBlock>(&_t_body_raw_io, Some(self._root.clone()), Some(self._self_shared.clone()))?.into();
...
pub struct Ext2_DirEntry {
    pub(crate) _root: SharedType<Ext2>,
    pub(crate) _parent: SharedType<Ext2_Dir>,
    pub(crate) _self_shared: SharedType<Self>,
...
impl TryFrom<i64> for Ext2_DirEntry_FileTypeEnum {
    ...
    fn try_from(flag: i64) -> KResult<Ext2_DirEntry_FileTypeEnum> {
        match flag {
            ...
            _ => Ok(Ext2_DirEntry_FileTypeEnum::UnknownVariant(flag)),
        }
    }
}
...
impl From<&Ext2_DirEntry_FileTypeEnum> for i64 {
    fn from(v: &Ext2_DirEntry_FileTypeEnum) -> Self {
        match *v {
            ...
            Ext2_DirEntry_FileTypeEnum::UnknownVariant(v) => v
        }
    }
}

impl Default for Ext2_DirEntry_FileTypeEnum {
    fn default() -> Self { Ext2_DirEntry_FileTypeEnum::UnknownVariant(0) }
}
...
pub struct Ext2_Inode {
    pub(crate) _root: SharedType<Ext2>,
    pub(crate) _parent: SharedType<Ext2_Bgd>,
    pub(crate) _self_shared: SharedType<Self>,
    mode: RefCell<u16>,
    uid: RefCell<u16>,
    size: RefCell<u32>,
...
pub struct Ext2_RawBlock {
    pub(crate) _root: SharedType<Ext2>,
    pub(crate) _parent: SharedType<Ext2_BlockPtr>,
    pub(crate) _self_shared: SharedType<Self>,
    body: RefCell<Vec<u8>>,
    _io: RefCell<BytesReader>,
}
