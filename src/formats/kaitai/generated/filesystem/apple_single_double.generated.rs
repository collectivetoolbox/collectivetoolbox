...
pub struct AppleSingleDouble {
    pub(crate) _root: SharedType<AppleSingleDouble>,
    pub(crate) _parent: SharedType<AppleSingleDouble>,
    ...
    pub fn body(
        ...
        match *self.r#type() {
            AppleSingleDouble_Entry_Types::FinderInfo => {
                *self.body_raw.borrow_mut() = _io.read_bytes(usize::try_from(*self.len_body())?)?.into();
                let body_raw = self.body_raw.borrow();
                let _t_body_raw_io = BytesReader::from(body_raw.clone());
                let t = Self::read_into::<BytesReader, AppleSingleDouble_FinderInfo>(&_t_body_raw_io, Some(self._root.clone()), Some(self._self_shared.clone()))?.into();
                *self.body.borrow_mut() = Some(t);
            }
            _ => {
                *self.body.borrow_mut() = Some(_io.read_bytes(usize::try_from(*self.len_body())?)?.into());
            }
        }
        _io.seek(_pos)?;
        Ok(self.body.borrow())
...
pub struct AppleSingleDouble_FinderInfo {
    pub(crate) _root: SharedType<AppleSingleDouble>,
    pub(crate) _parent: SharedType<AppleSingleDouble_Entry>,
    pub(crate) _self_shared: SharedType<Self>,
...
pub struct AppleSingleDouble_Point {
    pub(crate) _root: SharedType<AppleSingleDouble>,
    pub(crate) _parent: SharedType<AppleSingleDouble_FinderInfo>,
    pub(crate) _self_shared: SharedType<Self>,
    x: RefCell<u16>,
    y: RefCell<u16>,
    _io: RefCell<BytesReader>,
