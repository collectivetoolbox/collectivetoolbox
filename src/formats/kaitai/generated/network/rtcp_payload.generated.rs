...
pub struct RtcpPayload {
    pub(crate) _root: SharedType<RtcpPayload>,
    pub(crate) _parent: SharedType<RtcpPayload>,
        ...
        *self_rc.rtcp_packets.borrow_mut() = Vec::new();
        {
            let mut _i = 0;
            while !_io.is_eof() {
                let t = Self::read_into::<_, RtcpPayload_RtcpPacket>(&*_io, Some(self_rc._root.clone()), Some(self_rc._self_shared.clone()))?.into();
...
impl RtcpPayload_PacketStatusChunk {
    pub fn s(
        ...
        self.f_s.set(true);
        *self.s.borrow_mut() = (if (if *self.t() { 1 } else { 0 }) == 0 { (*self.s2()) as u64 } else { (if (if *self.s1() { 1 } else { 0 }) == 0 { (1) as i32 } else { (0) as i32 }) as u64 }).try_into()?;
        Ok(self.s.borrow())
    }
}
impl RtcpPayload_PacketStatusChunk {
...
pub struct RtcpPayload_PsfbAfbPacket {
    pub(crate) _root: SharedType<RtcpPayload>,
    pub(crate) _parent: SharedType<RtcpPayload_PsfbPacket>,
    pub(crate) _self_shared: SharedType<Self>,
    uid: RefCell<u32>,
        ...
        match *self_rc.uid() {
            1380273474 => {
                *self_rc.contents_raw.borrow_mut() = _io.read_bytes_full()?.into();
                ...
                let t = Self::read_into::<BytesReader, RtcpPayload_PsfbAfbRembPacket>(&_t_contents_raw_io, Some(self_rc._root.clone()), Some(self_rc._self_shared.clone()))?.into();
...
pub struct RtcpPayload_PsfbAfbRembPacket {
    pub(crate) _root: SharedType<RtcpPayload>,
    pub(crate) _parent: SharedType<RtcpPayload_PsfbAfbPacket>,
    pub(crate) _self_shared: SharedType<Self>,
    num_ssrc: RefCell<u8>,
    br_exp: RefCell<u64>,
...
impl RtcpPayload_PsfbAfbRembPacket {
    pub fn max_total_bitrate(
        ...
        *self.max_total_bitrate.borrow_mut() = ((((*self.br_mantissa()) as u64) * (((((1) as u64) << ((*self.br_exp()) as u64))) as u64))).try_into()?;
...
pub struct RtcpPayload_PsfbPacket {
    pub(crate) _root: SharedType<RtcpPayload>,
    pub(crate) _parent: SharedType<RtcpPayload_RtcpPacket>,
    pub(crate) _self_shared: SharedType<Self>,
    ssrc: RefCell<u32>,
        ...
        match *self_rc.fmt()? {
            RtcpPayload_PsfbSubtype::Afb => {
                *self_rc.fci_block_raw.borrow_mut() = _io.read_bytes_full()?.into();
                ...
                let t = Self::read_into::<BytesReader, RtcpPayload_PsfbAfbPacket>(&_t_fci_block_raw_io, Some(self_rc._root.clone()), Some(self_rc._self_shared.clone()))?.into();
...
impl RtcpPayload_ReportBlock {
    pub fn cumulative_packets_lost(
        ...
        *self.cumulative_packets_lost.borrow_mut() = ((((*self.lost_val()) as u64) & ((16777215) as u64))).try_into()?;
    ...
    pub fn fraction_lost(
        ...
        *self.fraction_lost.borrow_mut() = ((((*self.lost_val()) as i32) >> ((24) as i32))).try_into()?;
...
impl KStruct for RtcpPayload_RtcpPacket {
    ...
    fn read<S: KStream>(
        ...
        *self_rc.subtype.borrow_mut() = _io.read_bits_int_be(5)?;
        io.align_to_byte()?;
        *self_rc.payload_type.borrow_mut() = i64::try_from(_io.read_u1()?)?.try_into()?;
        ...
        match *self_rc.fmt()? {
            RtcpPayload_RtpfbSubtype::TransportFeedback => {
                *self_rc.fci_block_raw.borrow_mut() = _io.read_bytes_full()?.into();
                let fci_block_raw = self_rc.fci_block_raw.borrow();
                let _t_fci_block_raw_io = BytesReader::from(fci_block_raw.clone());
                let t = Self::read_into::<BytesReader, RtcpPayload_RtpfbTransportFeedbackPacket>(&_t_fci_block_raw_io, Some(self_rc._root.clone()), Some(self_rc._self_shared.clone()))?.into();
                *self_rc.fci_block.borrow_mut() = Some(t);
            }
            _ => {
                *self_rc.fci_block.borrow_mut() = Some(_io.read_bytes_full()?.into());
...
pub struct RtcpPayload_RtpfbTransportFeedbackPacket {
    pub(crate) _root: SharedType<RtcpPayload>,
    pub(crate) _parent: SharedType<RtcpPayload_RtpfbPacket>,
    pub(crate) _self_shared: SharedType<Self>,
...
impl RtcpPayload_RtpfbTransportFeedbackPacket {
    pub fn fb_pkt_count(
        ...
        *self.fb_pkt_count.borrow_mut() = ((((*self.b4()) as u32) & ((255) as u32))).try_into()?;
    ...
    pub fn packet_status(
        ...
        *self.packet_status.borrow_mut() = _io.read_bytes(usize::try_from(0)?)?;
    ...
    pub fn recv_delta(
        ...
        *self.recv_delta.borrow_mut() = _io.read_bytes(usize::try_from(0)?)?;
    ...
    pub fn reference_time(
        ...
        *self.reference_time.borrow_mut() = ((((*self.b4()) as u32) >> ((8) as u32))).try_into()?;
...
impl RtcpPayload_SdesPacket {
    pub fn source_count(
        ...
        *self.source_count.borrow_mut() = (*self._parent.get_value().borrow().upgrade().as_ref().ok_or(KError::MissingParent)?.subtype()).try_into()?;
...
pub struct RtcpPayload_SdesTlv {
    pub(crate) _root: SharedType<RtcpPayload>,
    pub(crate) _parent: SharedType<RtcpPayload_SourceChunk>,
    pub(crate) _self_shared: SharedType<Self>,
    r#type: RefCell<RtcpPayload_SdesSubtype>,
    length: RefCell<u8>,
...
impl KStruct for RtcpPayload_SdesTlv {
    ...
    fn read<S: KStream>(
        ...
        *self_rc.r#type.borrow_mut() = i64::try_from(_io.read_u1()?)?.try_into()?;
        ...
        if *self_rc.r#type() != RtcpPayload_SdesSubtype::Pad {
            *self_rc.value.borrow_mut() = _io.read_bytes(usize::try_from(*self_rc.length())?)?;
        }
        Ok(())
    }
}
...
impl RtcpPayload_SdesTlv {
    pub fn r#type(&self) -> Ref<'_, RtcpPayload_SdesSubtype> {
        self.r#type.borrow()
    }
}
impl RtcpPayload_SdesTlv {
...
pub struct RtcpPayload_SourceChunk {
    pub(crate) _root: SharedType<RtcpPayload>,
    pub(crate) _parent: SharedType<RtcpPayload_SdesPacket>,
    pub(crate) _self_shared: SharedType<Self>,
    ssrc: RefCell<u32>,
        ...
        *self_rc.sdes_tlv.borrow_mut() = Vec::new();
        {
            let mut _i = 0;
            while !_io.is_eof() {
                let t = Self::read_into::<_, RtcpPayload_SdesTlv>(&*_io, Some(self_rc._root.clone()), Some(self_rc._self_shared.clone()))?.into();
...
impl RtcpPayload_SrPacket {
    pub fn ntp(
        ...
        self.f_ntp.set(true);
        *self.ntp.borrow_mut() = (((((((*self.ntp_msw()) as u32) << ((32) as u32))) as u32) & ((*self.ntp_lsw()) as u32))).try_into()?;
        Ok(self.ntp.borrow())
    }
}
impl RtcpPayload_SrPacket {
