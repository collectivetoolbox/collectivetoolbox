...
pub struct Xar {
    pub(crate) _root: SharedType<Xar>,
    pub(crate) _parent: SharedType<Xar>,
...
impl Xar {
    ...
    pub fn checksum_algorithm_other(
        ...
        *self.checksum_algorithm_other.borrow_mut() = (3).try_into()?;
...
impl KStruct for Xar_FileHeader {
    ...
    fn read<S: KStream>(
        ...
        *self_rc._io.borrow_mut() = io.clone();
        ...
        self_rc._self_shared.set(Ok(self_rc.clone()));
        let _io = io;
        *self_rc.version.borrow_mut() = _io.read_u2be()?;
