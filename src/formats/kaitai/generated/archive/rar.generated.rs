...
pub struct Rar {
    pub(crate) _root: SharedType<Rar>,
    pub(crate) _parent: SharedType<Rar>,
...
impl KStruct for Rar {
    ...
    fn read<S: KStream>(
        ...
        *self_rc._io.borrow_mut() = io.clone();
        ...
        let _io = io;
        let t = Self::read_into::<_, Rar_MagicSignature>(&*_io, Some(self_rc._root.clone()), Some(self_rc._self_shared.clone()))?.into();
        ...
        *self_rc.blocks.borrow_mut() = Vec::new();
        {
            ...
            while !_io.is_eof() {
                match *self_rc.magic().version() {
                    0 => {
                        let _t_blocks_raw = _io.read_bytes_full()?;
                        let _t_blocks_raw_io = BytesReader::from(_t_blocks_raw);
                        let t = Self::read_into::<BytesReader, Rar_Block>(&_t_blocks_raw_io, Some(self_rc._root.clone()), Some(self_rc._self_shared.clone()))?.into();
                        self_rc.blocks.borrow_mut().push(t);
                    }
                    1 => {
                        let _t_blocks_raw = _io.read_bytes_full()?;
                        let _t_blocks_raw_io = BytesReader::from(_t_blocks_raw);
                        let t = Self::read_into::<BytesReader, Rar_BlockV5>(&_t_blocks_raw_io, Some(self_rc._root.clone()), Some(self_rc._self_shared.clone()))?.into();
                        self_rc.blocks.borrow_mut().push(t);
                    }
                    _ => {}
                }
...
impl KStruct for Rar_Block {
    ...
    fn read<S: KStream>(
        ...
        *self_rc.block_size.borrow_mut() = _io.read_u2le()?;
        if *self_rc.has_add()? {
            *self_rc.add_size.borrow_mut() = _io.read_u4le()?;
        }
        match *self_rc.block_type() {
            Rar_BlockTypes::FileHeader => {
                *self_rc.body_raw.borrow_mut() = _io.read_bytes_full()?.into();
                ...
                let t = Self::read_into::<BytesReader, Rar_BlockFileHeader>(&_t_body_raw_io, Some(self_rc._root.clone()), Some(self_rc._self_shared.clone()))?.into();
        ...
        if *self_rc.has_add()? {
            *self_rc.add_body.borrow_mut() = _io.read_bytes(usize::try_from(*self_rc.add_size())?)?;
        }
        Ok(())
    }
}
...
pub struct Rar_BlockFileHeader {
    pub(crate) _root: SharedType<Rar>,
    pub(crate) _parent: SharedType<Rar_Block>,
    pub(crate) _self_shared: SharedType<Self>,
