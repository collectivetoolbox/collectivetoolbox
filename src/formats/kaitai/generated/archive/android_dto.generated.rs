...
pub struct AndroidDto {
    pub(crate) _root: SharedType<AndroidDto>,
    pub(crate) _parent: SharedType<AndroidDto>,
...
impl AndroidDto_DtTableEntry {
    ...
    pub fn body(
        ...
        let _pos = io.pos();
        io.seek(usize::try_from(*self.dt_offset())?)?;
        *self.body.borrow_mut() = io.read_bytes(usize::try_from(*self.dt_size())?)?;
        io.seek(_pos)?;
        Ok(self.body.borrow())
    }
}
