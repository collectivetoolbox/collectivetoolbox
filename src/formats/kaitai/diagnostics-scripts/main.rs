...
//! Diagnostics script for validating Kaitai format compilation.
...
fn write_module_tree(src_dir: &Path, active_files: &[(String, String)]) -> std::io::Result<()> {
    ...
    // 1. generated.rs
    let mut gen_rs = String::from("// SPDX-License-Identifier: AGPL-3.0-or-later\n/*\nThis file is part of Collective Toolbox, a database and document workspace and utilities.\nCopyright (C) 2026 Collective Toolbox Developers\nContact: info@collectivetoolbox.com\n\nThis program is free software: you can redistribute it and/or modify it under\nthe terms of the GNU Affero General Public License as published by the Free\nSoftware Foundation, either version 3 of the License, or (at your option) any\nlater version.\n\nThis program is distributed in the hope that it will be useful, but WITHOUT ANY\nWARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR\nA PARTICULAR PURPOSE.  See the GNU Affero General Public License for more details.\n\nYou should have received a copy of the GNU Affero General Public License along\nwith this program.  If not, see <https://www.gnu.org/licenses/>.\n*/\n\n//! Generated Kaitai format modules.\n\n// @generated by ctb-formats-kaitai\n#![allow(unused_imports, clippy::wildcard_imports, overflowing_literals)]\n\n");
...
"#;
    ...
    // Reason for fallback: when system parallelism cannot be determined, default to 8 worker threads
                    ...
                    loop {
                        ...
                        match rustc_res {
                            ...
                            Ok(output) => {
                                ...
                                // Reason for fallback: if rustc output has no standard error prefix, report generic unknown error
    ...
    for (stem, info) in formats_arc.iter() {
        if !verified.contains(stem) {
            // Reason for fallback: missing last error description defaults to Unknown
