...
impl Elf_EndianElf_DynsymSection {
    pub fn is_string_table_linked(
        ...
        *self.is_string_table_linked.borrow_mut() = (*_prc.as_ref().unwrap().linked_section()?.r#type() == Elf_ShType::Strtab) as bool;
...
impl KStruct for Elf_EndianElf_DynsymSectionEntry {
    ...
    fn read<S: KStream>(
        ...
        *self_rc.r#type.borrow_mut() = (_io.read_bits_int_be(4)? as i64).try_into()?;
        _io.align_to_byte()?;
...
impl KStruct for Elf_EndianElf_NoteSectionEntry {
    ...
    fn read<S: KStream>(
        ...
        *self_rc.r#type.borrow_mut() = if *self_rc._is_le.borrow() == 1 { _io.read_u4le()?.into() } else { _io.read_u4be()?.into() };
...
impl KStruct for Elf_EndianElf_ProgramHeader {
    ...
    fn read<S: KStream>(
        ...
        *self_rc._io.borrow_mut() = _io.clone();
        ...
        let _prc = self_rc._parent.get_value().borrow().upgrade();
        let _r = _rrc.as_ref().unwrap();
        *self_rc.r#type.borrow_mut() = (_io.read_u4()? as i64).try_into()?;
        if *_r.bits() == Elf_Bits::B64 {
