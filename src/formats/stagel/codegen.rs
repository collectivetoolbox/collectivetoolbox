...
fn codegen_print_indentation_spaces(
    ...
    for _ in 0..codegen_indent {
        if force || *codegen_new_line {
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "    ");
        }
    }
}
...
fn codegen_literal(
    ...
    if typ == "literal-s" {
        ...
        codegen_string_literal_delim(target_lang, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
        if target_lang == "bash" || target_lang == "sh" {
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &temp);
        } else {
            let escaped = temp.replace('\\', "\\\\");
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &escaped);
        }
        codegen_string_literal_delim(target_lang, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
    } else if typ == "literal-n" || typ == "literal-b" {
        if target_lang == "bash" || target_lang == "sh" {
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("'{}'", val));
        } else {
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, val);
        }
...
fn codegen_reference(
    ...
    if target_lang == "js" {
        let formatted_type = codegen_format_type(target_lang, typ, false, false)?;
        codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &formatted_type);
        codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &uppercase_name);
    } else if target_lang == "bash" || target_lang == "sh" {
        let is_array = typ.starts_with("ident-a") || typ == "ident-ga" || typ == "ident-gi";
        if is_array {
            if !is_assignment_target {
                ...
                codegen_output(
                    ...
                    &format!("\"$(join_by $'\\037' \"${{{}{}[@]}}\")\"", formatted_type, uppercase_name),
                );
            } else {
                let formatted_type = codegen_format_type(target_lang, typ, false, false)?;
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &formatted_type);
...
fn codegen_initialize_type(
    ...
    match t {
        "bool" => {
            codegen_literal(target_lang, "literal-b", "false", output, codegen_new_line, codegen_newline_looking_for_spaces)?;
        }
        "int" => {
            codegen_literal(target_lang, "literal-n", "0", output, codegen_new_line, codegen_newline_looking_for_spaces)?;
        }
        "str" => {
            codegen_string_literal_delim(target_lang, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
            codegen_string_literal_delim(target_lang, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
...
fn codegen_start_array(
    ...
    if target_lang == "js" {
        ...
        let in_arglist = state_stack.last().map(|s| s == "arglist").unwrap_or(false);
        let is_not_set = block_stack.last().map(|b| b.1 != "set").unwrap_or(true);
...
fn codegen_end_array(
    ...
    if target_lang == "js" {
        ...
        let in_arglist = state_stack.get(state_stack.len().saturating_sub(2)).map(String::as_str) == Some("arglist");
        let is_not_set = block_stack.last().map(|b| b.1 != "set").unwrap_or(true);
        if in_arglist && is_not_set {
            ")\"".to_string()
        } else {
...
fn codegen_array_entry_delimiter(target_lang: &str, end_of_line: bool) -> String {
    if target_lang == "js" {
        if end_of_line {
            ",".to_string()
        } else {
...
fn codegen_format_argument(
    ...
    if target_lang == "js" {
        ...
        if formatted_type == "boolArray" || formatted_type == "intArray" || formatted_type == "strArray" || formatted_type == "genericArray" || formatted_type == "genericItem" {
            codegen_output(
                output,
                codegen_new_line,
                codegen_newline_looking_for_spaces,
...
fn codegen_comment(
    ...
    codegen_print_indentation_spaces(output, codegen_new_line, codegen_newline_looking_for_spaces, codegen_indent, false);
    if target_lang == "js" {
        codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("/*{} */\n", comment_text));
    } else if target_lang == "bash" || target_lang == "sh" {
        codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("#{}\n", comment_text));
    }
    Ok(())
...
fn codegen_dedent_action(
    ...
    if target_lang == "js" {
        ...
        let grandparent_type = block_stack.get(len.saturating_sub(2)).map(|x| x.0.as_str()).unwrap_or("");
        if grandparent_type == "test-body-if" || grandparent_type == "test-body-elif" || grandparent_type == "test-body-else" {
            if next_token_content != "else" && next_token_content != "elif" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "fi\n");
            }
        } else if grandparent_type == "test-body-while" {
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "done\n");
        } else {
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "}\n");
        }
    }
...
fn block_stack_remove_last(block_stack: &mut Vec<(String, String)>) -> Result<()> {
    let len = block_stack.len();
    if len >= 2
        && block_stack.get(len.saturating_sub(1)).is_some_and(|b| b.1.is_empty() && b.0 == "plain-block")
        && block_stack.get(len.saturating_sub(2)).is_some_and(|b| b.0 == "test")
    {
        block_stack.pop();
    }
...
fn codegen_get_current_routine_type(
    ...
    while counter > 0 {
        ...
        let t = &block_stack.get(counter).context("Invalid block stack index")?.0;
...
fn codegen_routine_definition_pre_end(
    ...
    if target_lang == "js" || target_lang == "bash" {
        if codegen_routine_type == "ident-r-v" {
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "\n");
            codegen_print_indentation_spaces(output, codegen_new_line, codegen_newline_looking_for_spaces, codegen_indent.saturating_add(1), false);
        }
        if debug_build {
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "StageL_internalDebugStackExit");
            if target_lang == "bash" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, ";");
            } else {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "();");
...
fn codegen_routine_definition_start(
    ...
    if target_lang == "js" {
        codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("async function {}(", codegen_routine_name));
    ...
    *codegen_indent = (*codegen_indent).saturating_sub(1);
    ...
    if target_lang == "bash" {
        for (k, (arg_type, arg_name)) in codegen_arg_list.iter().enumerate() {
            ...
            let is_array = formatted_type == "boolArray" || formatted_type == "intArray" || formatted_type == "strArray" || formatted_type == "genericArray" || formatted_type == "genericItem";
            ...
            if is_array {
                codegen_output(
                    ...
                    codegen_new_line,
                    codegen_newline_looking_for_spaces,
                    &format!("IFS=$'\\037' read -r -a {}{} <<< \"$1\"; shift", formatted_type, uppercase_name),
                );
            } else {
                if is_last && !debug_build {
                    codegen_output(
                        ...
                        codegen_new_line,
                        codegen_newline_looking_for_spaces,
                        &format!("{}{}=\"$1\"", formatted_type, uppercase_name),
                    );
                } else {
                    codegen_output(
                        output,
                        codegen_new_line,
                        codegen_newline_looking_for_spaces,
                        &format!("{}{}=\"$1\"; shift", formatted_type, uppercase_name),
                    );
                }
            }
            if !is_last || debug_build {
    ...
    if debug_build {
        for (k, (arg_type, arg_name)) in codegen_arg_list.iter().enumerate() {
            ...
            if target_lang == "js" {
                codegen_output(
                    ...
                    codegen_new_line,
                    codegen_newline_looking_for_spaces,
                    &format!("StageL_internalDebugCollect('{} {} = ' + {}{} + '; '); ", formatted_type, uppercase_name, formatted_type, uppercase_name),
                );
            } else if target_lang == "bash" {
                codegen_output(
                    output,
        ...
        if target_lang == "js" {
            codegen_output(
                ...
                codegen_new_line,
                codegen_newline_looking_for_spaces,
                &format!("StageL_internalDebugStackEnter('{}:{}');", codegen_routine_name, filename),
            );
        } else if target_lang == "bash" {
            codegen_output(
                output,
    ...
    if typecheck_build {
        for (k, (arg_type, arg_name)) in codegen_arg_list.iter().enumerate() {
            ...
            let formatted_type = codegen_format_type(target_lang, arg_type, true, false)?;
            if target_lang == "js" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("StageL_assertIs{}(", formatted_type));
                codegen_format_argument(target_lang, arg_type, arg_name, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, ");");
            } else if target_lang == "bash" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("StageL_assertIs{} ", formatted_type));
                codegen_format_argument(target_lang, arg_type, arg_name, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
    ...
    if codegen_routine_type != "ident-r-v" {
        if target_lang == "js" {
            let formatted_ret_type = codegen_format_type(target_lang, codegen_routine_type, false, false)?;
            codegen_output(
                output,
                codegen_new_line,
                codegen_newline_looking_for_spaces,
                &format!("let {}Return;", formatted_ret_type),
            );
        }
    }
...
fn codegen_command_invocation_arglist(
    ...
    for (k, (arg_type, arg_val)) in codegen_arg_list.iter().enumerate() {
        if arg_type.starts_with("literal-") {
            codegen_literal(target_lang, arg_type, arg_val, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
        } else {
            codegen_reference(target_lang, false, arg_type, arg_val, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
        }
        if k != argument_count.saturating_sub(1) {
            codegen_command_invocation_arg_separator(target_lang, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
        }
    }
...
fn codegen_command_invocation_end(
    ...
    if codegen_routine_name == "return" || codegen_routine_name == "new" {
        bail!("Internal error (this is a bug): codegenCommandInvocationEnd called for return/new");
    }
    if target_lang == "js" {
        let mut skip_paren = false;
        if let Some(last) = block_stack.last() {
            if last.0 == "command" && (last.1 == "new" || last.1 == "return" || last.1 == "set" || last.1 == "else" || last.1 == "debugger") {
                skip_paren = true;
            }
        }
        if !skip_paren {
        ...
        let last_block = block_stack.last().map(|b| b.1.as_str()).unwrap_or("");
        if last_block == "if" || last_block == "elif" || last_block == "while" {
            if last_block == "while" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, " ]]; do");
            } else {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, " ]]; then");
            }
        } else if *codegen_invocation_level != 0 {
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, ")\"");
            *codegen_invocation_level = (*codegen_invocation_level).saturating_sub(1);
        }
    }

    if !state_stack.contains(&"test".to_string()) {
        let is_not_arglist = state_stack.get(state_stack.len().saturating_sub(2)).map(|s| s != "arglist").unwrap_or(true);
        let last_block_is_not_else = block_stack.last().map(|b| b.0 != "else").unwrap_or(true);
...
fn codegen_command_invocation_start(
    ...
    match codegen_routine_name.as_str() {
        "new" => {
            if codegen_arg_list.iter().any(|arg| arg.0.starts_with("ident-r-")) {
                bail!("Sorry, invoking functions isn't available when declaring a variable. Please use set to assign it after declaration.");
            }
            codegen_print_indentation_spaces(output, codegen_new_line, codegen_newline_looking_for_spaces, codegen_indent, false);
            if target_lang == "js" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "let ");
            }
            let len = codegen_arg_list.len();
            if len >= 2 {
                let arg_minus_2 = codegen_arg_list.get(len.saturating_sub(2)).context("Missing arg minus 2")?;
                let arg_minus_1 = codegen_arg_list.get(len.saturating_sub(1)).context("Missing arg minus 1")?;
                codegen_reference(
                    target_lang,
                    true,
                    &arg_minus_2.0,
                ...
                if token_lookahead.starts_with("ident-") || token_lookahead.starts_with("literal-") {
                    if target_lang == "bash" {
                        codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "=");
                    } else {
                        codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, " = ");
                    }
                    *token_lookahead = "".to_string();
                } else {
                    let formatted = codegen_format_type(target_lang, &arg_minus_1.0, false, false)?;
                    if formatted != "generic" && formatted != "genericItem" {
                        codegen_initialize_type(target_lang, &formatted, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
                    }
                }
            }
        }
        "return" => {
            if codegen_arg_list.iter().any(|arg| arg.0.starts_with("ident-r-")) {
                bail!("Sorry, invoking functions isn't available when returning. Please assign the value to a variable and return that.");
            }
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "\n");
            codegen_print_indentation_spaces(output, codegen_new_line, codegen_newline_looking_for_spaces, codegen_indent, false);
            let return_type = codegen_get_current_routine_type(target_lang, block_stack)?;
            if return_type.is_empty() {
                bail!("Tried to return, but was not inside a routine!");
            }
            let formatted_ret_type = codegen_format_type(target_lang, &return_type, false, false)?;
            if target_lang == "js" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("{}Return = ", formatted_ret_type));
            } else if target_lang == "bash" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("{}Return=", formatted_ret_type));
            }
            let last_arg = codegen_arg_list.last().context("Missing last arg")?;
            let last_arg_type = &last_arg.0;
            let last_arg_val = &last_arg.1;
            if last_arg_type.starts_with("ident-") {
                codegen_reference(target_lang, false, last_arg_type, last_arg_val, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
            } else {
                codegen_literal(target_lang, last_arg_type, last_arg_val, output, codegen_new_line, codegen_newline_looking_for_spaces)?;
            }
            if typecheck_build {
                let formatted_upper = codegen_format_type(target_lang, &return_type, true, false)?;
                let formatted_lower = codegen_format_type(target_lang, &return_type, false, false)?;
                if target_lang == "js" {
                    codegen_output(
                        output,
                        codegen_new_line,
                        codegen_newline_looking_for_spaces,
                        &format!("; StageL_assertIs{}({}Return); ", formatted_upper, formatted_lower),
                    );
                } else if target_lang == "bash" {
                    codegen_output(
                        output,
            ...
            if target_lang == "js" {
                ...
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "print ");
                codegen_format_argument(
                    target_lang,
                    &return_type,
                    "return",
        ...
        "if" | "elif" | "while" => {
            ...
            if target_lang == "js" {
                if codegen_routine_name == "elif" {
                    codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, "else if (");
                } else {
                    codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("{} (", codegen_routine_name));
                }
            } else if target_lang == "bash" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("{} [[ \"true\" == ", codegen_routine_name));
            }
        }
        "else" | "debugger" => {
            codegen_print_indentation_spaces(output, codegen_new_line, codegen_newline_looking_for_spaces, codegen_indent, false);
            codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, codegen_routine_name);
        }
        "set" => {
            codegen_print_indentation_spaces(output, codegen_new_line, codegen_newline_looking_for_spaces, codegen_indent, false);
            let len = codegen_arg_list.len();
        ...
        _ => {
            codegen_print_indentation_spaces(output, codegen_new_line, codegen_newline_looking_for_spaces, codegen_indent, false);
            if target_lang == "js" {
                codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("StageL_{}(", codegen_routine_name));
            } else if target_lang == "bash" {
                if *codegen_new_line {
                    codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("StageL_{} ", codegen_routine_name));
                } else {
                    codegen_output(output, codegen_new_line, codegen_newline_looking_for_spaces, &format!("\"$(StageL_{} ", codegen_routine_name));
                    *codegen_invocation_level = (*codegen_invocation_level).saturating_add(1);
                }
            }
        }
    }
...
pub fn codegen(
    ...
    if target_lang != "js" && target_lang != "bash" {
        bail!("Target language {} is not supported", target_lang);
    }
                ...
                if i >= 1 && i + 4 < tokens.len() {
                    ...
                    let replace_suffix = |pos: &str, new_ind: usize| -> Result<String> {
                        if let Some(idx) = pos.rfind(':') {
                            Ok(format!("{}:{}", pos.get(..idx).context("Invalid pos prefix")?, new_ind))
                        } else {
                            Ok(pos.to_string())
                        }
                    };

                    let loop_elem_pos_indented = replace_suffix(&loop_elem.pos, next_indent)?;
                    let loop_idx_pos_indented = replace_suffix(&loop_idx.pos, next_indent)?;
    ...
    let mut codegen_routine_type = "".to_string();
    let mut codegen_routine_name = "".to_string();
    ...
    let mut codegen_array_literal = "".to_string();
    let mut state_stack = vec!["root".to_string(), "code".to_string()];
    let mut block_stack = vec![("root".to_string(), "".to_string())];
    ...
    while j < tokens.len() {
            ...
            && !state_stack.last().map(|s| s.starts_with("literal-a")).unwrap_or(false)
        {
            ...
            codegen_array_literal.push_str(&codegen_start_array(target_lang, &state_stack, &block_stack, "b"));
            state_stack.push("literal-ab".to_string());
        } else if current_token.typ == "literal-an-start" {
            codegen_array_literal.push_str(&codegen_start_array(target_lang, &state_stack, &block_stack, "n"));
            state_stack.push("literal-an".to_string());
        } else if current_token.typ == "literal-as-start" {
            codegen_array_literal.push_str(&codegen_start_array(target_lang, &state_stack, &block_stack, "s"));
            ...
            match current_state.as_str() {
                "literal-ab" => {
                    match current_token.typ.as_str() {
                        "literal-ab-end" => {
                            codegen_array_literal.push_str(&codegen_end_array(target_lang, &state_stack, &block_stack));
                            state_stack.pop();
                        ...
                        "newline" => {
                            codegen_array_literal.push_str(&codegen_array_entry_delimiter(target_lang, true));
                            codegen_array_literal.push('\n');
                            for _ in 0..codegen_indent {
                                codegen_array_literal.push_str("    ");
                            }
                        }
                        _ => bail!("Unexpected token {} in array of booleans.", current_token.typ),
                    }
                }
                "literal-an" => {
                    match current_token.typ.as_str() {
                        ...
                        _ => bail!("Unexpected token {} in array of numbers.", current_token.typ),
                    }
                }
                "literal-as" => {
                    match current_token.typ.as_str() {
                        ...
                        _ => bail!("Unexpected token {} in array of strings.", current_token.typ),
                    }
                }
                "code" => {
                    match current_token.typ.as_str() {
                        ...
                        "dedent" => {
                            if block_stack.last().map(|b| b.0 == "ident-r-v").unwrap_or(false) {
                                codegen_routine_definition_pre_end(
                                    target_lang,
                            ...
                            if block_stack.last().map(|b| b.0.starts_with("test-body-")).unwrap_or(false) {
                                block_stack_remove_last(&mut block_stack)?;
                            }
                        }
                        typ if typ.starts_with("ident-r-") => {
                            ...
                            block_stack.push((current_token.typ.clone(), current_token.content.clone()));
                        }
                        "command" => {
                            ...
                            match current_token.content.as_str() {
                                "if" | "elif" | "else" | "while" => {
                                    if current_token.content == "else" {
                                        block_stack.push((format!("test-body-{}", current_token.content), "".to_string()));
                                        ...
                                        block_stack.push((format!("test-body-{}", current_token.content), "".to_string()));
                                        block_stack.push(("test".to_string(), current_token.content.clone()));
                                        codegen_command_invocation_start(
                                            target_lang,
                                            &mut codegen_routine_name,
                                            &mut codegen_routine_type,
                                ...
                                "new" | "set" => {
                                    ...
                                    block_stack.push(("command".to_string(), current_token.content.clone()));
                                    state_stack.push("identifier-command-argument-accumulation".to_string());
                                }
                                "return" => {
                        ...
                        _ => bail!(
                            ...
                            block_stack.get(block_stack.len().saturating_sub(2)).map(|x| x.0.as_str()).unwrap_or(""),
                            block_stack.last().map(|x| x.0.as_str()).unwrap_or("")
                        ),
                    }
                }
                "oneshot-command" => {
                    ...
                    if is_arr_end {
                        ...
                        codegen_arg_list.push((current_token.typ.clone(), current_token.content.clone()));
                ...
                "arglist" | "test" => {
                    ...
                    if is_arr_end {
                        ...
                        codegen_arg_list.push((current_token.typ.clone(), current_token.content.clone()));
                    } else if current_token.typ == "command" || current_token.typ.starts_with("ident-r-") {
                        let cond1 = current_token.content == "return" || current_token.content == "new" || current_token.content == "set";
                        let cond2 = current_state == "test" && test_parameter_found;
                        if cond1 || cond2 {
                            ...
                            if current_state != "test" {
                                ...
                                if !tokens.get(j.saturating_sub(1)).map_or(false, |t| codegen_is_test_command(&t.content)) {
                            ...
                            state_stack.push("arglist".to_string());
                            block_stack.push(("arglist-command".to_string(), current_token.content.clone()));
                        }
                    } else if current_token.typ.starts_with("ident-") {
                        if current_state == "test" && test_parameter_found {
                            bail!(
                ...
                "routine-definition" => {
                    if current_token.typ.starts_with("ident-") {
                        codegen_arg_list.push((current_token.typ.clone(), ascii_dec_list_to_text(&current_token.content)));
                    } else if current_token.typ == "indent" || current_token.typ == "newline" {
...
use super::*;
    ...
    fn test_parse_and_codegen() {
        ...
        let parsed_html = crate::parse::parse(&html_content, "format-html").unwrap();
    ...
    fn test_parse_and_codegen_ascii() {
        let ascii_content = get_stagel_data("fixtures/format-ascii.stagel").unwrap();

        let parsed_ascii = crate::parse::parse(&ascii_content, "format-ascii").unwrap();
    ...
    fn compare_fixture(base_name: &str) {
        let input_path = format!("fixtures/{}.stagel", base_name);
        ...
        let expected_parsed_path = format!("fixtures/{}.stagel.parsed", base_name);
        ...
        if parsed != expected_parsed {
            ...
            assert_eq!(parsed_str, expected_str, "Parsed mismatch for {}", base_name);
        }
        ...
        for target_lang in &["js", "bash"] {
            for debug in &[true, false] {
                for typechecks in &[true, false] {
                    let generated = codegen(&parsed, target_lang, *debug, *typechecks).unwrap();

                    let debug_str = if *debug { "debug" } else { "nodebug" };
                    let typechecks_str = if *typechecks { "typechecks" } else { "notypechecks" };
                    let expected_codegen_path = format!(
                        "fixtures/{}.stagel.out-{}-{}-{}",
                        base_name, target_lang, debug_str, typechecks_str
                    );

                    let expected_codegen = get_stagel_data(&expected_codegen_path)
                        .unwrap_or_else(|| panic!("Failed to load {}", expected_codegen_path));

                    if generated != expected_codegen {
                        let generated_str = String::from_utf8_lossy(&generated);
                        let expected_str = String::from_utf8_lossy(&expected_codegen);
                        assert_eq!(
                            generated_str, expected_str,
                            "Codegen mismatch for {} (lang: {}, debug: {}, typechecks: {})",
                            base_name, target_lang, debug, typechecks
                        );
                    }
                }
            }
