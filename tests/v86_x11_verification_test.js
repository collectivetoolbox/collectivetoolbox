...
(async () => {
  ...
  console.log(`[VERIFY-X11] Connecting to ctoolbox server at ${serverUrl}...`);
  await waitForServer(serverUrl, 60000);
  console.log("[VERIFY-X11] Server is up and responsive!");
  ...
  const targetUrl = `${serverUrl}/v86/guix`;
  console.log(`[VERIFY-X11] Navigating to ${targetUrl}...`);
  await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
  await page.waitForLoadState('load');

  console.log("[VERIFY-X11] Page loaded. Waiting for v86 emulator initialization...");
  ...
  console.log("[VERIFY-X11] Waiting for Guix VM kernel / REPL boot (up to 60s)...");
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (state.text.includes('scheme@(guile-user)>') || state.serial.includes('scheme@') || state.serial.includes('login:') || state.serial.includes('root@') || state.text.includes('openbox') || state.text.includes('#')) {
      bootPromptSeen = true;
      console.log(`[VERIFY-X11] Detected Guix boot prompt after ${i+1}s!`);
      break;
    }
  }

  if (!bootPromptSeen) {
    console.log("[VERIFY-X11] Warning: Boot prompt signature not seen within 60s, proceeding to test gates...");
  } else {
    console.log("[VERIFY-X11] Pausing 35s for desktop startup script & 9p chunk loading...");
    await page.waitForTimeout(35000);
  }

  console.log("\n==============================================");
  console.log("STARTING 5-GATE X11 & OPENBOX VERIFICATION");
  console.log("==============================================\n");
  ...
  console.log("\n==============================================");
  console.log("SYSTEM & X11 DIAGNOSTIC DUMP");
  console.log("==============================================");
  ...
  const screenshotPath = '/workspaces/ctoolbox/built/v86_x11_verified_desktop.png';
  ...
  if (!allPassed) {
    console.error("\n==============================================");
    console.error("X11 VERIFICATION RESULT: FAILED");
  ...
  console.log("\n==============================================");
  console.log("X11 VERIFICATION RESULT: SUCCESS! ALL 6 GATES PASSED!");
  console.log("Guix v86 X11 Server + Openbox desktop verified working!");
  console.log("==============================================\n");
  process.exit(0);
})();
